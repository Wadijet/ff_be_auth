...
// Organization đại diện cho cấu trúc tổ chức hình cây (Tập đoàn, Công ty, Phòng ban, Bộ phận, Team)
type Organization struct {
	...
	Type      string              `json:"type" bson:"type" index:"single:1"`                        // Loại tổ chức (group, company, department, division, team)
	ParentID  *primitive.ObjectID `json:"parentId,omitempty" bson:"parentId,omitempty" index:"single:1"` // ID tổ chức cha (null nếu là root)
	Path      string              `json:"path" bson:"path" index:"single:1"`                        // Đường dẫn cây (ví dụ: "/root_group/company1/dept1")
	Level     int                 `json:"level" bson:"level" index:"single:1"`                      // Cấp độ (0 = root, 1, 2, ...)
