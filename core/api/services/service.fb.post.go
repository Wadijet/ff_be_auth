...
func NewFbPostService() (*FbPostService, error) {
	fbPostCollection, exist := registry.Collections.Get(global.MongoDB_ColNames.FbPosts)
	if !exist {
		return nil, fmt.Errorf("failed to get fb_posts collection: %v", utility.ErrNotFound)
	}
